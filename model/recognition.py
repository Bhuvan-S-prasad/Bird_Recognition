...
# Load class names (species) from a file or define them directly
...
# Function to predict bird species for a single image
def predict_bird(image_path):
    ...
    bird_name = class_names[predicted.item()]
    return bird_name

# Function to predict bird species for all images in a directory
def predict_birds_in_directory(directory_path):
    results = {}
    for filename in os.listdir(directory_path):
        if filename.endswith(".jpg") or filename.endswith(".png"):
            image_path = os.path.join(directory_path, filename)
            bird_name = predict_bird(image_path)
            results[filename] = bird_name
            print(f"Image: {filename}, Predicted Bird: {bird_name}")
    return results

# Example usage
image_path = "C:/Users/bhuva/Desktop/Red_Winged_Blackbird_0054_4625.jpg"
bird_name = predict_bird(image_path)
print(f'The predicted bird species is: {bird_name}')
